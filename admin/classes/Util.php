<?php
    ...
    class Util
    {
		...
		function getKeywords($expr = '', $min = 3)
		{
		...
		function cleanString( $string, $rep = '-', $numeric = true, $special = true, $extra = '', $drep = false )
		{
        ...
        function removeFunnyCharacters($str)
        {
            $table = array(
        ...
        function cauta($content,$strStart,$strStop)
        {
            $response     = array();
		...
		function search($content,$strStart,$strStop, $canbeempty = true, $ind = false){
			$response=array();
			$new_content2=$content;
			while(strpos($new_content2,$strStart) !== false){
				$pos1=strpos($new_content2,$strStart);
        ...
        function GetCurrentUrl( $base_url = "" )
        {
            $ret  = $base_url."?";
            $arr  = $_GET;
            $vals = array_keys($arr);
        ...
        function GetCurrentUrlHTTP()
        {
            $ret  = "http://".$_SERVER["SERVER_NAME"].$_SERVER["REQUEST_URI"];
            return $ret;
        }

        function GetCurrentUrlWithoutKeys($keys)
        {
        ...
        function GetCurrentUrlReplacingVal($base_url, $key, $value)
        {
            $ret = $base_url."?";
            $arr = $_GET;
            $vals= array_keys($arr);
