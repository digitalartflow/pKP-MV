<?php
	...
	if( !empty( $_SESSION['state_postalcode']['State'] ) && !empty( $_SESSION['state_postalcode']['PostalCode'] ) ){
		...
		if( $list->Size() > 0 ){
			...
			$where .= ' and ( u.Regions like "%{'.$state_postalcode['Id'].'}%" or u.Regions = "" or u.Regions IS NULL  )  ';
	...
	select 
		...
		u.RetailPricePercentage as RetailPricePercentage, 
		u.Swf2D as Swf2D, 
...
//echo $sql;
...

	{
		...
		if( isset( $_SESSION['logged']['DealerPricePercentage'] ) && $_SESSION['logged']['DealerPricePercentage'] > 0 )
			...
			$item['FinalPrice'] = $item['FinalPrice']*(1+$_SESSION['logged']['DealerPricePercentage']/100);

		else

			$item['FinalPrice'] = $item['FinalPrice']*(1+$item['RetailPricePercentage']/100);
