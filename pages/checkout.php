<?
	...
	if( !empty( $_SESSION['shoppingCart']['u'] ) )
		...
		{
			$item = array();
			$list = new DbList('select uv.Id, uv.IdUnit, uv.Code, uv.PriceDiff, u.Name, u.Price, u.RetailPricePercentage from unit_variations as uv, units as u where uv.Id="'.$u.'" and uv.IdUnit=u.Id');
			$uv   = $list->Get(0);
			$item['Id'] 			= $uv['Id'];
			$item['Name'] 			= $uv['Name'];
			$item['Code'] 			= $uv['Code'];
			...
			if( isset( $_SESSION['logged']['DealerPricePercentage'] ) && $_SESSION['logged']['DealerPricePercentage'] > 0 )
				$item['Price'] = $item['Price']*(1+$_SESSION['logged']['DealerPricePercentage']/100);
			else
				$item['Price'] = $item['Price']*(1+$uv['RetailPricePercentage']/100);
	...
	if( !empty( $_POST['action'] ) && $_POST['action'] == "checkout" )
	{
		...
		//mail($_SESSION['logged']['Email'],'Order information',$msg,$header);
		mail($_SESSION['logged']['Email'],'Order information!! #'.$nid,$msg,$header);
		mail(WEBSITE_CONTACT_EMAIL,'Order information!! #'.$nid,$msg,$header);
